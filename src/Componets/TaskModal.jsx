...
const TaskModal = ({  closeModal, task, addTask, updateTask }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (task) {
        response = await updateTask({ id: task._id, title, description, status:"pending" });
      } else {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white p-6 rounded-lg shadow-lg w-10/12">
        <h2 className="text-xl font-bold mb-4">{task ? "Update Task" : "Add Task"}</h2>
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          ...
          <button type="button" onClick={closeModal} className="bg-gray-500 text-white px-4 py-2 rounded shadow hover:bg-gray-600">
            Cancel
          </button>
        </form>
      </div>
